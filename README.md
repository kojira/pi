...
- Release smoke tests use `pnpm run release:local` to build, pack, and create isolated pnpm and Bun installs outside the repo before tagging a release.
- Local release installs, documented pnpm installs, and `pi update --self` use `--ignore-scripts` where supported.
