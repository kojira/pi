...
function bumpOrSetVersion(target) {
	...
	if (BUMP_TYPES.has(target)) {
		...
		run(`pnpm -r version ${target} --no-git-tag-version && node scripts/sync-versions.js && pnpm install --lockfile-only --ignore-scripts && npm install --package-lock-only --ignore-scripts`);
	}

	// Keep pnpm as the install source of truth while refreshing the npm lock that
	// still feeds published npm shrinkwrap and installer-lock artifacts.
