...
Pi compaction and branch summaries using this model still return an explicit error; use a supported model for summarization. This is independent of model switching and does not delete existing Pi history. Steer and multiple concurrent actions have not been proven equivalent to the default model. The adapter changes only two Pi documentation instructions in the SDK request while retaining Pi identity and document paths; it refuses unexpected source changes rather than silently dropping them. No SDK `maxTurns` or estimated USD trial cap is imposed; the official account's own usage limits still apply.
