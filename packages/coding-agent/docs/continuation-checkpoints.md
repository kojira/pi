...
A successful `continue_work` creates a persisted active contract and returns its checkpoint ID. Active requests use `tool_choice: "auto"`, allowing either tools or a terminal structured text decision. Ordinary work tools and user comments do not clear the contract. The model can resolve it with `finish_work` alone, supplying `checkpointId`, `outcome` (`completed`, `cancelled`, `waiting`, or `blocked`), `reason`, and `summary`.

Alternatively, a text response can end with a standalone `<work-control>{"action":"continue","nextAction":"Run verification"}</work-control>` line, or `<work-control>{"action":"finish","outcome":"completed","reason":"Verification passed"}</work-control>`. The preceding text is the visible finish summary. The runtime strips the footer and normalizes the model's decision into the existing work-tool lifecycle. Continue can begin a new checkpoint; finish requires an active one. No synthetic user message or second classifier is introduced. See [text work control](text-work-control.md).

Main assistant responses in this opt-in mode are buffered until normalization, including responses containing ordinary tools. This intentionally trades token-by-token display for preventing control suffixes from leaking through RPC, print or UI events. Auxiliary summaries are not buffered by this adapter.
...
An active text-only response missing a valid decision gets at most two correction turns through the existing checkpoint boundary, then an explicit protocol error. Fenced and quoted markers never resolve work; truncated controls cannot finish. A footer mixed with tool calls is rejected before execution. Abort, disposal, provider error, repair exhaustion, and settlement without a finish decision do not mean completion. Active persisted records restore as suspended and are never automatically executed. Contract records survive transcript compaction.
