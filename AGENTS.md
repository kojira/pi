...
- If dep metadata changes, refresh `pnpm-lock.yaml` with `pnpm install --lockfile-only --ignore-scripts`, then refresh the npm artifact lock with `npm install --package-lock-only --ignore-scripts` because shrinkwrap/install-lock generation still emits npm lockfiles for published consumers.
